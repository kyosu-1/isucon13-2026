...
func getUserStatisticsHandler(c echo.Context) error {
	...
	var user UserModel
	if err := sqlx.GetContext(ctx, tx, &user, "SELECT * FROM users WHERE name = ?", username); err != nil {
    ...
    WHERE u.name = ?
	`
	if err := sqlx.GetContext(ctx, tx, &totalReactions, query, username); err != nil && !errors.Is(err, sql.ErrNoRows) {
	...
	var totalTip int64
	if err := tx.QueryRowxContext(ctx, `SELECT COUNT(*), IFNULL(SUM(c.tip), 0) FROM livecomments c INNER JOIN livestreams l ON l.id = c.livestream_id WHERE l.user_id = ?`, user.ID).Scan(&totalLivecomments, &totalTip); err != nil && !errors.Is(err, sql.ErrNoRows) {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to get livecomments: "+err.Error())
	}

	// 合計視聴者数
	var viewersCount int64
	if err := sqlx.GetContext(ctx, tx, &viewersCount, `SELECT COUNT(*) FROM livestream_viewers_history h INNER JOIN livestreams l ON l.id = h.livestream_id WHERE l.user_id = ?`, user.ID); err != nil && !errors.Is(err, sql.ErrNoRows) {
	...
	LIMIT 1
	`
	if err := sqlx.GetContext(ctx, tx, &favoriteEmoji, query, username); err != nil && !errors.Is(err, sql.ErrNoRows) {
...
func getLivestreamStatisticsHandler(c echo.Context) error {
	...
	if err := sqlx.GetContext(ctx, tx, &livestream, "SELECT * FROM livestreams WHERE id = ?", livestreamID); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
	...
	// 視聴者数算出
	var viewersCount int64
	if err := sqlx.GetContext(ctx, tx, &viewersCount, `SELECT COUNT(*) FROM livestreams l INNER JOIN livestream_viewers_history h ON h.livestream_id = l.id WHERE l.id = ?`, livestreamID); err != nil && !errors.Is(err, sql.ErrNoRows) {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to count livestream viewers: "+err.Error())
	}

	// 最大チップ額
	var maxTip int64
	if err := sqlx.GetContext(ctx, tx, &maxTip, `SELECT IFNULL(MAX(tip), 0) FROM livestreams l INNER JOIN livecomments l2 ON l2.livestream_id = l.id WHERE l.id = ?`, livestreamID); err != nil && !errors.Is(err, sql.ErrNoRows) {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to find maximum tip livecomment: "+err.Error())
	}

	// リアクション数
	var totalReactions int64
	if err := sqlx.GetContext(ctx, tx, &totalReactions, "SELECT COUNT(*) FROM livestreams l INNER JOIN reactions r ON r.livestream_id = l.id WHERE l.id = ?", livestreamID); err != nil && !errors.Is(err, sql.ErrNoRows) {
		return echo.NewHTTPError(http.StatusInternalServerError, "failed to count total reactions: "+err.Error())
	}

	// スパム報告数
	var totalReports int64
	if err := sqlx.GetContext(ctx, tx, &totalReports, `SELECT COUNT(*) FROM livestreams l INNER JOIN livecomment_reports r ON r.livestream_id = l.id WHERE l.id = ?`, livestreamID); err != nil && !errors.Is(err, sql.ErrNoRows) {
