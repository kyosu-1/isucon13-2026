...
### 19:58〜20:02 仕上げ（いずれも ±0、CPU が減る/余裕が増えるので採用）

- GOGC=400 → 705805
- nginx のアクセスログを 64KB バッファ・1 秒 flush → 701772
- 配信者サブドメインを isu2:isu3 = 1:1（isu1 は app + DNS に専念）→ 696749

同一コードのブレは 64〜71万。最高 708197。ベンチ機（8 vCPU、本番相当）が負荷後半に busy 73% で、
サーバー側の CPU（isu1 68% / isu2 58% / isu3 64%）はどれも飽和していない。**残る律速は 1 リクエストの遅延と
ベンチ側のペース（予約枠が 10 秒で埋まる、視聴者の投入レート）**で、アプリ側でできることは小さくなった。

### 20:04 最終の再起動試験 → 690793（pass）

最終構成で 3台同時 reboot → 全サービス自動起動 → pass。環境は TTL（05:05 ごろ）まで残す。

### 20:10〜20:35 方針に沿った追跡（ユーザー指示「対応を進めて」）

**2峰性の分析（`tools/analyze/timeline.py`）**: 遅い回（644130）は負荷開始 30 秒以降に視聴者投入・予約試行・
アイコン変更が一斉に 3 割減り、そのとき isu3（pipe 宛 nginx）の CPU だけが 77→81% に**上がる**
（速い回は 60% に下がる）。リクエストあたりの nginx の仕事が増えている＝TLS ハンドシェイクを疑った。

**接続ログ（`tools/analyze/conns.sh`、access log に conn / conn_req / ssl_reused / proto を追加）**:
- 新規接続は 1000〜2000/5秒で、**全部 TLS フルハンドシェイク**（ベンチはセッション再開を使わない）。
  10〜40 秒は登録・ログイン（新規ユーザー = 新規接続）で 2 倍
- HTTP/2 は登録・ログインの 8k 件だけ。無効化しても接続パターンは不変（703161）
- 証明書は RSA-2048（署名 ≒ 0.7ms/回、変えられない）

**pipe の A レコードをラウンドロビン（TTL 1 秒）にしてハンドシェイクを 3 台に分散**: 接続は分散したが
650986 / 665072 と固定 isu3（70 万）より低い → 無効化（機能は `ISUCON13_DNS_PIPE_ADDRESS` に残す）。
固定 isu3 の構成はすでに 3 台が 60〜70% で均等だった。

**HTTP/2 無効化後は 6 回連続 697〜703k**（無効化前は 3 回に 1 回 64 万台）。遅い回は HTTP/2 由来の可能性が高い。

**守り**: `proxy_cache_lock off`（icon の max 0.5s → 0.012s）。最終構成で再起動試験 → 682448（pass）。

### 20:45 最終計測 → 705235（最終スコア）

ユーザーの判断で現状を最終形とする。最終構成・計測 OFF（slow log なし）で最後の 1 回。
直前の再起動試験は 682448（再現率 97%、追試の基準 75% を満たす）。
環境は TTL（05:05 ごろ）で自動削除。作り直す場合は README の手順で `make setup`。
