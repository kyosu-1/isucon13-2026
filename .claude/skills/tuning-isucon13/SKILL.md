...
ベンチは初期化 3s + 整合性チェック 5s + 負荷 60s で**約 70 秒**（ツール込みで 2.5 分）。まとめて入れる理由はない。
...
| 新規接続は全部 TLS フルハンドシェイク（再開なし）、1000〜2000/5秒 | RSA-2048 の署名が nginx の CPU の主成分。証明書は変えられない | conns-*.txt |
| pipe をラウンドロビン（TTL 1）で分散しても伸びない | 3 台が 60〜70% で均等ならそれ以上は動かない | 20260922-202053 |
| HTTP/2 を切ると 64 万台の遅い回が出なくなった（6 回連続 70 万） | HTTP/2 は off のまま | journal 20:35 |
