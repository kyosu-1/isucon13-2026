...
# 再起動試験。
#
# ISUCON13の追試は「ポータルに表示されている3台のサーバーを再起動」→「負荷走行」。
...
[ $# -eq 0 ] && set -- isucon13-1 isucon13-2 isucon13-3
HOSTS=("$@")
...
echo "==> サービス確認"
for HOST in "${HOSTS[@]}"; do
  ssh "${SSH_OPTS[@]}" "$HOST" "printf '  %-11s ' \$(hostname); for s in nginx mysql pdns isupipe-go; do
