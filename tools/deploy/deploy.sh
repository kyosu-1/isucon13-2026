...
#   build/isupipe            -> /home/isucon/webapp/go/isupipe
#   webapp/sql/              -> /home/isucon/webapp/sql/      (POST /api/initialize が init.sh を叩く)
#   webapp/pdns/             -> /home/isucon/webapp/pdns/     (init.sh が init_zone.sh を叩く)
#   etc/isuN/nginx/          -> /etc/nginx/
#   etc/isuN/mysql/          -> /etc/mysql/                   (変更があったときだけ mysql を再起動)
#   etc/isuN/powerdns/       -> /etc/powerdns/                (変更があったときだけ pdns を再起動)
...
MANAGED="nginx mysql pdns isupipe-go"

[ -x build/isupipe ] || { echo "build/isupipe がありません。make build を先に実行してください" >&2; exit 1; }
...
deploy_one() {
  ...
  rsync -az --rsync-path="$RS_ISUCON" build/isupipe "$HOST:$REMOTE/go/isupipe"
  rsync -az --rsync-path="$RS_ISUCON" webapp/sql/ "$HOST:$REMOTE/sql/"
  rsync -az --rsync-path="$RS_ISUCON" webapp/pdns/ "$HOST:$REMOTE/pdns/"

  local changed_nginx="" changed_mysql="" changed_pdns="" changed_systemd="" changed_env=""
  if [ -d "$dir/nginx" ]; then
  ...
  if [ -d "$dir/powerdns" ]; then
    changed_pdns="$(rsync -rlpcz -i --rsync-path='sudo rsync' "$dir/powerdns/" "$HOST:/etc/powerdns/" | grep -v '^\.' || true)"
  fi
  ...
  local want="$MANAGED"
  [ -f "$dir/services" ] && want="$(grep -v '^#' "$dir/services" | xargs || true)"

  {
    [ -n "$changed_nginx$changed_mysql$changed_pdns$changed_systemd$changed_env" ] && \
      echo "    [$HOST] 設定変更: ${changed_nginx:+nginx }${changed_mysql:+mysql }${changed_pdns:+pdns }${changed_systemd:+systemd }${changed_env:+env.sh}"

    ssh "$HOST" "set -euo pipefail
      sudo chown root:root -R /etc/nginx /etc/mysql /etc/powerdns 2>/dev/null || true
      ...
      if systemctl is-active --quiet nginx; then
        printf 'api=%s' \$(curl -sk -o /dev/null -w '%{http_code}' --resolve pipe.u.isucon.local:443:127.0.0.1 'https://pipe.u.isucon.local/api/tag')
      fi
      if systemctl is-active --quiet pdns; then
        printf ' dns=%s' \$(dig +short +time=2 pipe.u.isucon.local @127.0.0.1 | head -1)
      fi
      echo
    "
